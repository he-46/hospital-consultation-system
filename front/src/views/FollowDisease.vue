  ...
  <div class="follow-page">
    ...
    <div class="container main-content">
      ...
      <div class="content-card">
        <h2 class="page-title">关注的疾病</h2>
        <div v-loading="loading">
          <el-empty v-if="!loading && list.length === 0" description="暂未关注任何疾病" />
          <div v-else class="follow-grid">
            <div v-for="item in list" :key="item.id" class="follow-card" @click="$router.push('/disease/' + item.followId)">
              <div class="card-body">
                <h4>{{ item.diseaseName }}
                  <el-tag v-if="item.diseaseAlias" size="small" type="warning">{{ item.diseaseAlias }}</el-tag>
                </h4>
                <p class="desc">{{ item.diseaseDesc || '暂无描述' }}</p>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await getFollows({ page: currentPage.value, size: pageSize.value, followType: 3 })
...
.desc { color: #666; font-size: 13px; margin-bottom: 12px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
