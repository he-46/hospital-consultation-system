  ...
  <div class="follow-page">
    ...
    <div class="container main-content">
      ...
      <div class="content-card">
        <h2 class="page-title">关注的医生</h2>
        <div v-loading="loading">
          <el-empty v-if="!loading && list.length === 0" description="暂未关注任何医生" />
          <div v-else class="follow-grid">
            <div v-for="item in list" :key="item.id" class="follow-card" @click="$router.push('/doctor/' + item.followId)">
              <el-image :src="item.doctorAvatar" fit="cover" class="card-img" style="height:140px">
                <template #error><div class="img-placeholder"><el-icon :size="48"><User /></el-icon></div></template>
              </el-image>
              <div class="card-body">
                <h4>{{ item.doctorName }} <el-tag size="small">{{ item.doctorTitle }}</el-tag></h4>
                <p class="dept">{{ item.doctorDepartment || '-' }}</p>
                <p class="intro">{{ item.doctorIntro || '暂无介绍' }}</p>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await getFollows({ page: currentPage.value, size: pageSize.value, followType: 2 })
...
.follow-card { border: 1px solid #eee; border-radius: 8px; overflow: hidden; cursor: pointer; transition: box-shadow .2s; }
.follow-card:hover { box-shadow: 0 2px 12px rgba(0,0,0,.1); }
.card-body { padding: 14px; }
.card-body h4 { margin: 0 0 6px; font-size: 16px; }
.dept { color: #1e88e5; font-size: 13px; margin-bottom: 8px; }
...
.img-placeholder { width: 100%; height: 140px; background: #f5f7fa; display: flex; align-items: center; justify-content: center; color: #bbb; }
